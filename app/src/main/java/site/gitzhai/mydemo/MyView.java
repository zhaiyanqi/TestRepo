...
public class MyView extends View {
    ...
    private void drawTouchPoint(Canvas canvas) {
        if (mbTouchDown) {
            ...
            float curK = 1.138f - (1.138f - k) * mTouchProgress;
...
//                if (checkPoint(a, b, c, d, 0, r1)) {
//                    PointF result = getDisconnectY(a, b, c, d);
//                    boolean disconnect = Float.compare(result.x, 0) > 0 && Float.compare(result.y, 0) > 0;
//
////                    if (disconnect) {
//////                    drawDisconnect(canvas, mModel, result);
////                    } else {
////                    }
////                drawConnect(canvas, mModel);
//                }
    ...
    private void drawConnect(Canvas canvas, Model model) {
        ...
        for (float dy = 0; dy <= model.d + (model.ball1.r); dy = dy + 6) {
            for (float dx = model.maxX; dx >= 0; dx = dx - 1) {
                if (checkPoint(model.a, model.b, model.c, model.d, dx, dy)) {
                    path1.lineTo(model.ball1.x - dx, model.ball1.y + dy);
                    path2.lineTo(model.ball1.x + dx, model.ball1.y + dy);
                    canvas.drawCircle(model.ball1.x - dx - 1, model.ball1.y + dy, 1, mPaint2);
                    canvas.drawCircle(model.ball1.x + dx + 1, model.ball1.y + dy, 1, mPaint2);
...
//        }
    ...
    private boolean checkPoint(float a, float b, float c, float d, float dx, float dy) {
        float x2 = dx*dx;
        float y2 = dy*dy;
        double result = (b / Math.sqrt(x2+y2)) + (c / Math.sqrt(x2+ (d - dy) * (d - dy))) - a;
        return Double.compare(result, 10) > 0;
    }
